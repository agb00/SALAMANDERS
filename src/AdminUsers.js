...
function AdminSchedule() {
  ...
  // 일정 데이터 불러오기: schedule_items 테이블에서 데이터를 가져옵니다.
  useEffect(() => {
    ...
    fetch(`http://localhost:4000/api/schedule`, {
      ...
      .then(data => {
        ...
        const filteredSchedules = data.map(item => {
          const itemDate = new Date(item.week_start);
          itemDate.setHours(itemDate.getHours() + 9); // UTC -> KST 변환
          ...
          return { ...item, week_start: formattedItemDate };
        });

        // 현재 주 시작일에 맞는 일정만 필터링
        const currentWeekItems = filteredSchedules.filter(item => item.week_start === currentWeekStart);
